...
void LCD_writeText(char * message,unsigned short x, unsigned short y, unsigned short color,
        unsigned short bckcolor){
    char msg[25];
    sprintf(msg,message);
    int u;
    __builtin_disable_interrupts();
    for(u=0; msg[u]!='\0'&(x+5*u)<128;u++){
        LCD_writeChar(msg[u],x+5*u,y,color,bckcolor);
    }
